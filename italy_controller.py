...
class VehicleController:
    ...
    def get_next_traj_point(self, current_point,n):
        """
        提取当前轨迹点之后的下一个轨迹点
        """
        ...
        if current_index is not None and current_index + 1 < len(self.traj):
            return self.traj[current_index + n]["data"]["position"]
        else:
    ...
    def update(self):
        ...
        if current_time - self.previous_time >= 0.1:
            ...
            next_way_point = self.get_next_traj_point(current_way_point,5)
            ...
            if(self.distance(pos, next_way_point) > 0.1):
                ...
                steering = 2*steering_angle / math.pi
                steering = -steering



                if speed_error > 0 or speed <0.5:  # 如果当前速度低于目标速度
                    # 增加油门
                    throttle = min(0.8, speed_error * 0.1)  # 最大油门设为0.8
                    brake = 0.0  # 不需要刹车
                elif speed_error < 0:  # 如果当前速度高于目标速度
                    # 减少油门
                    throttle = max(0.01, speed_error * 0.1)  # 最小油门为0.01
                    brake = min(0.5, -speed_error * 0.1)  # 如果速度过快，则适当刹车
                else:  # 如果速度误差在允许范围内
                    throttle = 0.01  # 保持最低油门以维持车速
                    brake = 0.0  # 不需要刹车

                if abs(steering) > 0.5 :  # 如果转向角度大于0.5弧度，认为是较大转弯
                    # 减速策略：转向过大时，减小油门并适当加刹车
                    throttle = 0.01  # 减小油门
                    brake = 0.2  # 加强刹车力度
                    print("Large steering angle detected, reducing speed")
